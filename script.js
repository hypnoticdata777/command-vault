...
const quoteCategories = [...new Set(quotes.map(q => q.category))];

function getRandomQuotes(count = 5, category = null) {
...
// Now comes the rest of your existing script.js
...
function showHelp() {
  appendOutput(`<span style="color:#00ff00">Commands:</span> help, clear, quote, ${quoteCategories.join(', ')}`);
  appendOutput(`<span style="color:#888">Type a category name to get 5 quotes in English and Spanish.</span>`);
}
...
function handleCommand(command) {
  ...
  if (cmd === 'help') {
    ...
    const selected = getRandomQuotes(5);
    selected.forEach(q => {
      appendOutput(`<span style="color:#ffc600">» ${q.en}</span><br><span style="color:#999">• ${q.es}</span><br>`);
    });
  } else if (quoteCategories.includes(cmd)) {
    const selected = getRandomQuotes(5, cmd);
    selected.forEach(q => {
      appendOutput(`<span style="color:#ffc600">» ${q.en}</span><br><span style="color:#999">• ${q.es}</span><br>`);
    });
  } else {
    appendOutput(`<span style="color:red">Unknown command: ${cmd}</span>`);
  }
}
